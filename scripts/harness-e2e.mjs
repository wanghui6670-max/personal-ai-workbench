...
const bridge=http.createServer(async(req,res)=>{
  ...
  if(body.method==='initialize')result={protocolVersion:'2025-06-18',capabilities:{tools:{listChanged:false}},serverInfo:{name:'unified-workbench-e2e',version:'2.0.0'}};
    ...
    if(body.params?.name!=='project_list'){res.writeHead(200,{'content-type':'application/json'});res.end(JSON.stringify({jsonrpc:'2.0',id:body.id,error:{code:-32601,message:'unexpected tool'}}));return;}
  ...
  res.writeHead(200,{'content-type':'application/json'});res.end(JSON.stringify({jsonrpc:'2.0',id:body.id??null,result}));
});
...
const harness=new DeepSeekHarness({
  launch:{command:process.execPath,args:[path.join(harnessDir,'runtime-bin.mjs'),path.join(harnessDir,'navigator.test.cordis.yml')],cwd:harnessDir,env:childEnv,requestTimeoutMs:30_000,shutdownTimeoutMs:1500,disposeEofGraceMs:6000,disposeGraceMs:3000},
  cwd:root,provider:'joycrew',model:'navigator-replay',maxTokens:512
});
function diagnostic(events){return events.filter(event=>['assistant/message','tool/call','tool/result','turn/end'].includes(event.type)).map(event=>({type:event.type,data:event.data}));}
try{
  const first=await harness.run('请查看项目');
  assert.equal(first.finalResponse,'已读取 1 个项目：端到端测试项目。');
  if(calls.length!==1)console.error('Harness E2E diagnostic:',JSON.stringify({rpcTrace,calls,events:diagnostic(first.events)},null,2));
