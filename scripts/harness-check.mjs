...
function smokeSchema(name){
  if(name==='panel_navigate'){
    return {
      ...
      properties:{view:{type:'string',enum:['today','project']},id:{anyOf:[{type:'string',minLength:1},{type:'null'}]}},
      required:['view']
    };
  }
...
const bridge=http.createServer(async(req,res)=>{
  ...
  const result=body.method==='tools/list'
    ?{tools:SMOKE_TOOL_NAMES.map(name=>({name,description:`compile smoke ${name}`,inputSchema:smokeSchema(name),readOnly:true,requiresConfirmation:false}))}
    :body.method==='tools/call'
      ?{content:[{type:'text',text:'[]'}],structuredContent:{result:[],readback:true}}
      :{};
...
const childEnv={
  ...
  HARNESS_PROVIDER_API_KEY:'compile-smoke-not-a-real-secret',
  HARNESS_PROVIDER_MODEL:'compile-smoke-model',
  HARNESS_PROVIDER_API:'openai-responses',
  HARNESS_PROVIDER_BASE_URL:'https://example.invalid/v1',
  HARNESS_PROVIDER_CONTEXT_WINDOW:'32768',
  HARNESS_PROVIDER_MAX_TOKENS:'512',
...
const harness=new DeepSeekHarness({
  ...
  cwd:root,
  provider:'joycrew',
  model:'compile-smoke-model',
  maxTokens:512
});

try{
  await harness.start();
  console.log('Harness Navigator composition compiled and initialized successfully.');
}finally{
