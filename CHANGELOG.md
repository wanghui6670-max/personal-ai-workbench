...
- 个人待办来源从飞书云文档切换为受控 `ticktick` CLI；国际版使用 `TICKTICK_HOST=ticktick.com`，国内版使用 `TICKTICK_HOST=dida365.com`
- 有截止日期的滴答任务按外部 task ID 映射为正式待办；无截止日期进入待处理收件箱；同步不自动加入今日，也不反向修改滴答任务
- 飞书《每日工作日记》改为任务快照和用户触发每日总结的沉淀目标，使用固定章节、固定前缀、operationId 查重和写后读回
- 新增私有本机 ICS 日历：只镜像源任务已有日期和时段，全天语义不被改写，不猜时长，完成任务在下次同步时移除
- AI/MCP 新增外部待办设置、同步和每日总结工具，移除旧 `feishu_inbox_sync` 白名单；浏览器设置和 `doctor` 同步更新
...
- AI 工具结果先显示人类可读摘要，原始 JSON 收入技术详情；项目同步、飞书同步、备份和 AI 执行增加可关闭的页内操作回执
