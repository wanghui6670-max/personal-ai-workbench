...
这是一个以业务板块和项目为上下文边界的团队 AI 工作台，部署在云服务器上，支持 5-10 人小团队使用。目标不是增加管理动作，而是让团队成员在频繁被打断后快速恢复工作现场，并把外部信息先收进同一个可确认的处理队列。每个成员拥有独立的数据空间和认证身份，管理员可以查看全团队成员的工作数据概要。

业务场景：金融行业投标业务（银行、券商、保险、金租、消金等招标项目）的全流程管理（商机发现→跟进→投标→归档），以及政企项目与产业园区（常州地区中以园、科教城、OPC 社区等）的投标机会跟进。
