...
它解决的核心问题是：工作被打断以后，不再重新找资料、重新理解上下文、重新开始。

## 核心原则

1. 所有新事情先进入收件箱。
2. AI 不自动分类收件箱；只有你下明确指令后才移动。
3. 项目是独立上下文边界。业务板块 → 项目 → 本地文件夹一一对应。
4. 本地项目文件夹保存真实工作产物；Git 保存版本证据。
5. 飞书项目云文档是项目分析、阶段总结、复盘和上下文恢复叙事的唯一真源。
6. Workbench 只保存最小机器状态、任务、确认项、路径和飞书记录指针。
7. 项目进度只在你主动点击同步时刷新，不做后台自动安排。
8. 待办必须有截止日期；没有日期必须追问。
9. AI 可以分析和提醒，但绝不能自动把任务加入今日工作台。
...
`PROJECT.md` 只是项目身份证和入口索引，不再保存项目分析、卡点或恢复摘要。
...
要求：Node.js 20+；如需 Git 证据，还需要安装 Git。启用飞书时还需要在同一台机器安装并登录 `lark-cli`。
...
默认 Profile 是 `openai_luna`，推理档位固定为 `xhigh`。三类业务工作流使用 Structured Outputs：项目创建、项目进度和早晨对话。

配置 Key、doctor 通过或设置页显示“已配置”，只证明参数准备就绪，不证明模型已经真实联网成功。Provider 超时、拒绝、不可达或结构校验失败时，业务函数回退本地规则，不会自动改变截止日期、收件箱或今日计划。

启用 Provider 后，项目描述、业务板块、项目/待办元数据、相对文件名和修改时间、Git 提交元数据及对话上下文可能发送给 Provider，并先做凭证脱敏。文件正文默认不出站；只有显式设置 `AI_SEND_FILE_CONTENT=1` 才会发送受支持正文。脱敏不是完整 DLP，不应对高敏感项目开启正文发送。
...
服务必须运行在能访问真实项目文件的机器上，例如你的电脑、Mac mini、NAS、办公室服务器或挂载真实目录的 Docker 主机。普通无状态云函数不能直接读取笔记本上的本地路径。

部署说明见 [`docs/DEPLOYMENT.md`](docs/DEPLOYMENT.md)。

## 飞书每日工作日记收件箱

工作台可以把收件箱绑定到飞书云文档《个人 AI 工作台｜每日工作日记》。只读取一级标题“收件箱”下以 `[INBOX]` 开头的段落或待办块。
...
飞书不可达或读回失败时，本地不会伪装成已保存。请在设置页填写你自己的飞书文档 URL；仓库不保存真实工作文档链接。
...
随后本地状态只保留机器字段，并把需要处理的项目放进待确认。原始正文不会被假装成已迁移到飞书。
...
这证明的是本地受限工具层已实现，不等同于外部 MCP Host、真实云端模型或飞书已经完成 live 验收。

## 安全

如果绑定到非 localhost 地址，程序默认要求 `WORKBENCH_PASSWORD`，否则拒绝启动。启用密码时必须设置至少 24 字符且非示例值的 `SESSION_SECRET`。
...
恢复脚本会先创建安全备份。备份默认与主数据在同一磁盘，不等同于异地灾备，也不包含真实项目工作区。
...
自动化测试使用 fake Provider 和 fake Feishu client，不请求或处理真实凭证，不等于 live OpenAI、飞书、浏览器、iPhone 或部署验证。
