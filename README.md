...
- Docker 部署
- 环境自检与自动化测试
- AI-native 双面板：左侧给人操作，右侧固定 AI 工作区；AI 通过白名单 MCP 工具预览、确认、执行并读回左侧状态
...
桌面端打开后，左侧是人的工作面板，右侧是固定的 AI 工作区。AI 不是另一个孤立页面：已配置的当前模型先通过 `ai_console` 结构化工作流提出一个白名单 MCP 工具调用，模型不直接执行；本地 MCP 注册表再做参数校验、确认门和领域规则执行。未配置模型或模型输出不合约时，右侧会显示“本地安全回退”，使用确定性规划器。只读请求可直接执行；写入请求必须先显示工具参数和影响范围，点击确认后才会执行，执行完成后重新读回左侧状态与工具结果。

本地 MCP-compatible transport：`POST /api/mcp`；工具清单：`GET /api/ai/tools`。这证明的是本项目内受限工具层和 JSON-RPC 入口已实现，不等同于已经连接外部 MCP Host 或真实云端模型。
