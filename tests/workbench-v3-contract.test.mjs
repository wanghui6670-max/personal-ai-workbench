...
test('v3 merges Today and Inbox with Feishu-first atomic todo extraction before execution',async()=>{
  const [index,script,registry,contentTools,scope,planner]=await Promise.all([
    ...
    fsp.readFile('src/diary-review-planner.mjs','utf8')
  ]);
  ...
  assert.match(script,/同步飞书并提取待办/);
  assert.match(script,/\/api\/inbox\/sync/);
  assert.match(script,/\/api\/ai\/plan/);
  assert.match(script,/\/api\/ai\/execute/);
  assert.match(script,/diary_extract_todos/);
  assert.match(script,/feishu_todo_candidate/);
  assert.match(script,/confirm-extracted-todo/);
  assert.match(script,/AI 待办提取/);
  assert.match(script,/确认创建待办/);
  assert.match(planner,/不要自动加入 Today/);
  assert.match(planner,/绝不能新建项目/);
  assert.match(planner,/不(?:要|得)删除或改写飞书原文/);
  ...
  assert.match(scope,/REVIEW_TOOLS=new Set\(\['inbox_process','diary_extract_todos'\]\)/);
...
test('Feishu auto extraction is bounded, scoped to one item and reuses unchanged session previews',async()=>{
  ...
  assert.match(script,/item\.source==='feishu_doc'/);
...
test('v3 enhancement observer cannot self-trigger through descendant rewrites',async()=>{
  ...
  assert.match(script,/button\.textContent!=='同步飞书'/);
});
