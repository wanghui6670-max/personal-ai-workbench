...
test('MCP exposes the new pipeline and retires Feishu as an inbox source',async t=>{
  ...
  await assert.rejects(
    registry.call('external_task_integration_update',{enabled:false,cliFlavor:'ticktick'}),
    error=>error.code==='MCP_CONFIRMATION_REQUIRED'
  );
});

test('local planner maps explicit Dida sync and daily summary commands without auto-executing',async t=>{
  const {registry}=await fixture(t);
  const sync=await registry.plan('同步滴答待办');
  ...
  assert.match(legacy.message,/待办来源是滴答清单 CLI/);
});
