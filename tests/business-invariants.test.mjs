...
test('real file changes do not alter persisted or derived progress before explicit sync',async t=>{
  withoutOpenAi(t);
  ...
  assert.deepEqual(derived.projects[0].progress,baseline);
...
test('successful AI morning judgment still preserves todayPlan and its date exactly',async t=>{
  ...
  assert.equal(result.reply,replyMarker,'must exercise the successful AI path, not the local fallback');
  assert.deepEqual(after.todayPlan,before.todayPlan);
  assert.equal(after.todayPlanDate,before.todayPlanDate);
});

test('AI analysis audit fields never persist to state or PROJECT.md',async t=>{
  const {appRoot,store,config}=await fixture(t);
  const p=project({id:'p_analysis_audit',name:'分析隔离项目',folder:'分析隔离项目'});
  const dir=await ensureProjectDir(appRoot,config,p);
  await store.updateState(state=>{state.projects.push(p);});
  const analysisSentinel='ANALYSIS_SENTINEL_DO_NOT_PERSIST';
  const decisionMarker='AI_DECISION_MARKER_MUST_PERSIST';
  installMockAi(t,{
  ...
  assert.equal(state.projects[0].progress.summary,decisionMarker,'the business decision must be used');
  assert.match(projectMd,new RegExp(decisionMarker));
  assert.doesNotMatch(JSON.stringify(state),new RegExp(analysisSentinel));
  assert.doesNotMatch(rawState,new RegExp(analysisSentinel));
  assert.doesNotMatch(projectMd,new RegExp(analysisSentinel));
});
...
test('AI cannot mark a project completed without the user completion flag',async t=>{
  ...
  await store.updateState(state=>{state.projects.push(p);});
  installMockAi(t,{
    percent:100,summary:'模型擅自判断完成',resume:'模型擅自判断完成',blocker:'暂无明确卡点。',status:'已完成',confidence:.99
  });
  ...
  assert.doesNotMatch(projectMd,/状态：已完成/);
  assert.doesNotMatch(projectMd,/百分比：100%/);
});
...
test('stale Luna analysis cannot overwrite a user completion made while sync is running',{timeout:5000},async t=>{
  ...
  assert.deepEqual(saved.progressBeforeCompletion,prior);
  ...
  assert.match(projectMd,/状态：已完成/);
  assert.match(projectMd,/百分比：100%/);
...
test('stale Luna analysis cannot undo a user reopening made while sync is running',{timeout:5000},async t=>{
  ...
  const projectMd=await fsp.readFile(path.join(dir,'PROJECT.md'),'utf8');
  assert.equal(saved.completed,false);
  assert.deepEqual(saved.progress,prior);
  assert.equal(Object.hasOwn(saved,'progressBeforeCompletion'),false);
  assert.equal(state.activities.some(item=>item.type==='project_synced'&&item.projectId===p.id),false);
  assert.doesNotMatch(JSON.stringify(state),new RegExp(staleMarker));
  assert.match(projectMd,/状态：进行中/);
  assert.match(projectMd,/百分比：42%/);
  assert.doesNotMatch(projectMd,/项目已标记完成/);
  assert.doesNotMatch(projectMd,new RegExp(staleMarker));
});
...
test('ordinary low-confidence AI progress creates a visible confirmation',async t=>{
  ...
  assert.equal(analysis.scan.complete,true,'confirmation must come from AI confidence, not a scan-budget fallback');
...
test('sync never recreates a missing project directory and persists a confirmation',async t=>{
  withoutOpenAi(t);
  const {appRoot,store,config}=await fixture(t);
  const prior={
    percent:42,status:'进行中',summary:'已经完成需求梳理。',resume:'正在实现核心流程。',blocker:'等待一项本地资料。',
    lastActivity:'2026-08-11T08:00:00.000Z',confidence:.8,syncedAt:'2026-08-11T09:00:00.000Z'
  };
  ...
  assert.equal(analysis.progress.percent,42,'missing directories must not erase the last known progress');
  assert.ok(analysis.progress.confidence<=.2);
  assert.match(state.projects[0].progress.summary,/不存在|不可访问/);
  assert.equal(state.confirmations.some(item=>item.type==='progress_unclear'&&item.projectId===p.id),true);
});

test('completed project restores its exact prior progress when reopened',async t=>{
  const {appRoot,store,config}=await fixture(t);
  const prior={
    percent:42,status:'进行中',summary:'需求和数据结构已经完成。',resume:'继续实现同步流程。',blocker:'等待确认一个边界。',
    lastActivity:'2026-08-11T08:00:00.000Z',confidence:.76,syncedAt:'2026-08-11T09:00:00.000Z'
  };
  ...
  const projectMd=await fsp.readFile(path.join(dir,'PROJECT.md'),'utf8');
  assert.match(projectMd,/百分比：42%/);
  assert.match(projectMd,/需求和数据结构已经完成/);
  assert.doesNotMatch(projectMd,/项目已标记完成/);
});

test('PROJECT.md write failures do not leave sync or project-edit state committed',async t=>{
  withoutOpenAi(t);
  const {appRoot,store,config}=await fixture(t);
  ...
  for(const [label,operation] of [
    ['sync',()=>syncProject({appRoot,store,projectId:p.id})],
    ['update',()=>updateProject({appRoot,store,projectId:p.id,patch:{intro:'不应提交的新介绍'}})]
  ]){
    const before=await store.readState();
    await assert.rejects(operation(),/托管区块不完整/,label);
    assert.deepEqual(await store.readState(),before,`${label} failure must leave persisted state unchanged`);
