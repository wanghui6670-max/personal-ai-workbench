...
export function deriveState(appRoot,state,config,aiEnabled=false){
  ...
  derived.projects=derived.projects.map(project=>{
    ...
    const recoveryPending=state.confirmations.some(item=>item.type==='project_record_recovery_pending'&&item.projectId===project.id);
    return {
      ...project,
      recordRecoveryPending:recoveryPending,
      progress:{
        ...machine,
        summary:recoveryPending?'飞书记录已经保存，但本地指针仍待对账。':bound?'机器进度已同步；项目分析正文保存在飞书项目文档。':'机器进度可用；项目尚未绑定飞书项目文档，分析正文不会在本地保存。',
...
export async function updateProject({appRoot,store,projectId,patch}){
  ...
  const normalizedFeishu=Object.hasOwn(patch,'feishu')?normalizeFeishuProjectDocumentUrl(patch.feishu,{allowEmpty:true}):undefined;

  let updated=null;
  await store.updateState(state=>{
    const project=state.projects.find(item=>item.id===projectId);
    if(!project)throw new Error('项目不存在');
    for(const field of ['intro','git'])if(Object.hasOwn(patch,field))project[field]=patch[field].trim();
    if(normalizedFeishu!==undefined&&normalizedFeishu!==(project.feishu||'')){
      ...
      if(project.progressBeforeCompletion)project.progressBeforeCompletion=clearProjectRecordPointer(project.progressBeforeCompletion);
      state.confirmations=state.confirmations.filter(item=>!(item.projectId===project.id&&['project_record_recovery_pending','project_feishu_missing'].includes(item.type)));
    }
    if(Object.hasOwn(patch,'completed')){
      ...
      if(project.completed&&!wasCompleted){
        ...
        if(project.progressBeforeCompletion&&typeof project.progressBeforeCompletion==='object'&&!Array.isArray(project.progressBeforeCompletion)){
          ...
          project.progress={...current,percent:Math.min(99,Number.isInteger(current.percent)?current.percent:0),status:current.lastActivity?'进行中':'未启动',hasBlocker:Boolean(current.hasBlocker)};
        }
...
function staleSyncError(){
  return Object.assign(new Error('项目在同步期间发生变化，本次过期分析未写入；请重新同步。'),{statusCode:409,code:'PROJECT_SYNC_STALE'});
}

function remoteSavedPendingError(receipt,cause){
  const error=new Error('飞书项目记录已保存，但本地机器状态尚未提交。系统已保留可恢复凭据，请重新同步或在待确认中对账。',{cause});
  error.statusCode=409;
  error.code='PROJECT_RECORD_REMOTE_SAVED_LOCAL_PENDING';
  error.recovery={operationId:receipt.operationId,revisionId:receipt.revisionId,blockId:receipt.blockId,recordedAt:receipt.recordedAt};
  return error;
}
...
function missingFeishuConfirmation(state,project){
  ...
  state.confirmations.unshift({id:newId('cf'),type:'project_feishu_missing',projectId:project.id,text:`「${project.name}」尚未绑定飞书项目文档。本次只保存机器进度，项目分析正文未落地。`,createdAt:nowIso()});
}
...
async function addRecoveryConfirmation(store,project,receipt){
  await store.updateState(state=>{
    if(state.confirmations.some(item=>item.type==='project_record_recovery_pending'&&item.operationId===receipt.operationId))return;
    state.confirmations.unshift({
      id:newId('cf'),type:'project_record_recovery_pending',projectId:project.id,operationId:receipt.operationId,
      text:`「${project.name}」有一条飞书记录已保存但本地指针未提交。operationId=${receipt.operationId}，需要重新同步或人工核对。`,createdAt:nowIso()
    });
  }).catch(()=>{});
}
...
async function directoryAvailable(dir){
  if(!dir)return false;
  try{const stat=await fsp.lstat(dir);return !stat.isSymbolicLink()&&stat.isDirectory();}
  catch{return false;}
}
...
function analysisOperationId(project,sourceDir,analysis){
  return projectRecordOperationId('analysis',{
    ...
    scan:{complete:analysis.scan?.complete??null,reasons:analysis.scan?.reasons||[],directoriesVisited:analysis.scan?.directoriesVisited??null,maxDepthVisited:analysis.scan?.maxDepthVisited??null}
  });
}

async function syncProjectUnlocked({appRoot,store,projectId,projectRecordClient=defaultProjectRecordClient}){
  ...
  if(projectRecordConfigured(project)){
    ...
    await store.writeProjectRecordReceipt({operationId,kind:'analysis',projectId:project.id,documentUrl:project.feishu,recordedAt,projectSnapshotHash,machineProgress:pendingMachine,phase:'remote_pending'});
    try{
      ...
      const committedMachine=machineProgress(analysis.progress,{revisionId:record.revisionId,item:record.item,recordedAt,operationId});
      receipt=await store.writeProjectRecordReceipt({operationId,kind:'analysis',projectId:project.id,documentUrl:project.feishu,revisionId:record.revisionId,blockId:record.item?.blockId,recordedAt,projectSnapshotHash,machineProgress:committedMachine,phase:'remote_saved_local_pending'});
    }catch(error){
  ...
  try{
    await store.updateState(current=>{
      ...
      const recordPointer=record?{revisionId:record.revisionId,item:record.item,recordedAt,operationId}:null;
      p.progress=machineProgress(analysis.progress,recordPointer);
      if(!p.git&&analysis.gitRemote)p.git=analysis.gitRemote;
      if(record){
        clearProjectRecordConfirmations(current,p.id);
        current.confirmations=current.confirmations.filter(item=>!(item.type==='project_record_recovery_pending'&&item.operationId===operationId));
      ...
      addActivity(current,{type:'project_synced',projectId:p.id,text:`同步「${p.name}」：${p.progress.percent}% · ${p.progress.status}${record?' · 分析正文已写入飞书':' · 未保存分析正文（未绑定飞书项目文档）'}`});
      committed=structuredClone(p);
    });
  }catch(error){
    if(record&&receipt){await addRecoveryConfirmation(store,project,receipt);throw remoteSavedPendingError(receipt,error);}
    throw error;
  }
  ...
  return {
    ...
    gitRemote:analysis.gitRemote||'',filesCount:analysis.filesCount||0,dir:analysis.dir||sourceDir,scan:analysis.scan||null,
    record:record?{saved:true,replayed:Boolean(record.replayed),documentUrl:project.feishu,revisionId:record.revisionId??null,blockId:record.item?.blockId??null,recordedAt,operationId}:{saved:false,documentUrl:project.feishu||null,recordedAt:null,operationId:null}
  };
}
...
async function syncAllProjectsUnlocked({appRoot,store,projectRecordClient=defaultProjectRecordClient}){
  const state=await store.readState();
  const results=[];
  for(const project of state.projects.filter(item=>item.businessId&&!item.archived)){
    try{
      const analysis=await syncProjectUnlocked({appRoot,store,projectId:project.id,projectRecordClient});
      results.push({id:project.id,ok:true,progress:analysis.machineProgress,record:analysis.record});
    }catch(error){
      if(error?.code==='PROJECT_SYNC_STALE')results.push({id:project.id,ok:false,stale:true,code:error.code});
      else if(error?.code==='PROJECT_RECORD_REMOTE_SAVED_LOCAL_PENDING')results.push({id:project.id,ok:false,remoteSaved:true,code:error.code,recovery:error.recovery});
      else{
        await store.updateState(current=>{
          if(!current.confirmations.some(item=>item.type==='sync_failed'&&item.projectId===project.id))current.confirmations.unshift({id:newId('cf'),type:'sync_failed',projectId:project.id,text:`「${project.name}」同步失败：${error.message}`,createdAt:nowIso()});
        });
...
export async function readProjectRecords({store,projectId,limit,beforeBlockId=null,projectRecordClient=defaultProjectRecordClient}){
  const project=await persistedProject(store,projectId);
  if(!project)throw new Error('项目不存在');
  if(!projectRecordConfigured(project))throw Object.assign(new Error('项目尚未绑定有效的飞书项目文档。'),{statusCode:409});
  ...
  return {
    projectId:project.id,documentUrl:project.feishu,revisionId:records.revisionId??null,nextCursor,
    records:page.map(item=>({blockId:item.blockId,kind:item.kind,operationId:item.operationId||null,text:item.text}))
  };
}
...
export async function appendProjectSummary({store,projectId,text,projectRecordClient=defaultProjectRecordClient}){
  ...
  const operationId=projectRecordOperationId('summary',{projectId,documentUrl:project.feishu,text:value,parentBlockId:project.progress?.feishuRecordBlockId||null});
  const recordedAt=nowIso();
  const projectSnapshotHash=snapshotHash({id:project.id,feishu:project.feishu});
  await store.writeProjectRecordReceipt({operationId,kind:'summary',projectId,documentUrl:project.feishu,recordedAt,projectSnapshotHash,machineProgress:project.progress||null,phase:'remote_pending'});
  let result;
  let receipt;
  try{
    result=await projectRecordClient.appendSummary(project.feishu,value,{operationId});
    const nextMachine=machineProgress(project.progress||{},{revisionId:result.revisionId,item:result.item,recordedAt,operationId});
    receipt=await store.writeProjectRecordReceipt({operationId,kind:'summary',projectId,documentUrl:project.feishu,revisionId:result.revisionId,blockId:result.item?.blockId,recordedAt,projectSnapshotHash,machineProgress:nextMachine,phase:'remote_saved_local_pending'});
  }catch(error){
    await store.deleteProjectRecordReceipt(operationId).catch(()=>{});
    throw error;
  }
  try{
    await store.updateState(state=>{
      ...
      current.progress=machineProgress(current.progress||{},{revisionId:result.revisionId,item:result.item,recordedAt,operationId});
      state.confirmations=state.confirmations.filter(item=>!(item.type==='project_record_recovery_pending'&&item.operationId===operationId));
  ...
  return {saved:true,replayed:Boolean(result.replayed),projectId,documentUrl:project.feishu,revisionId:result.revisionId??null,blockId:result.item?.blockId??null,recordedAt,operationId};
}
