...
export function createEndpointRateLimiter({limits=RATE_LIMIT_DEFAULTS.limits,windowMs=RATE_LIMIT_DEFAULTS.windowMs,maxClients=RATE_LIMIT_DEFAULTS.maxClients,now=Date.now}={}){
  ...
  const safeLimits=Object.fromEntries(Object.entries(RATE_LIMIT_DEFAULTS.limits).map(([scope,fallback])=>[
    scope,
    boundedInteger(limits?.[scope],fallback,{min:1,max:RATE_LIMIT_CAPS.limits[scope]})
  ]));
  ...
  function consume(scope,clientKey){
    ...
    if(!clientEntry){
      while(clients.size>=safeMaxClients){
      ...
      // Refresh insertion order so capacity eviction behaves as a small LRU cache.
...
function lease(onRelease){
  let released=false;
  return{release(){if(released)return;released=true;onRelease();}};
}
...
export function createSyncCoordinator(){
  ...
  return{
    tryAcquireAll(){
      if(allActive||activeProjects.size)return null;
      allActive=true;
      return lease(()=>{allActive=false;});
    },
    tryAcquireProject(projectId){
      const key=String(projectId);
      if(allActive||activeProjects.has(key))return null;
      activeProjects.add(key);
      return lease(()=>{activeProjects.delete(key);});
    },
