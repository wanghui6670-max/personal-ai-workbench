...
function fail(message,code='INVALID_GETNOTE_INSIGHT',statusCode=400){
  throw new GetnoteInsightError(message,{code,statusCode});
}
...
function confidence(value,label){
  ...
  if(!Number.isFinite(number)||number<0||number>1)fail(`${label} 必须是 0 到 1 之间的数字。`);
  return number;
}
function array(value,label,{max=200}={}){
  if(value===undefined)return[];
  if(!Array.isArray(value))fail(`${label} 必须是数组。`);
  if(value.length>max)fail(`${label} 数量不能超过 ${max}。`);
  return value;
}
...
function dateOnly(value,label){
  ...
  if(!match)fail(`${label} 必须是 YYYY-MM-DD 或 null。`);
...
function contentHash(value){
  ...
  if(!CONTENT_HASH.test(raw))fail('source.contentHash 必须是 sha256:<64 hex>。');
  return raw;
}
function canonicalEvidenceIds(ids){return [...new Set(ids.map(String))].sort();}

export function getnoteContentHash(content){
  return `sha256:${digest(String(content??''))}`;
}
...
export function insightCacheKeyFor({noteId,contentHash:hash,parserVersion,modelProfile}={}){
  const note=text(noteId,'noteId',{max:256});
  const sourceHash=contentHash(hash);
  const version=text(parserVersion,'parserVersion',{max:100});
  const profile=text(modelProfile,'modelProfile',{max:200});
  return `gni_${digest(JSON.stringify([note,sourceHash,version,profile])).slice(0,40)}`;
}
...
function normalizeSource(value){
  ...
  return{contentHash:contentHash(source.contentHash),language:text(source.language??'zh-CN','source.language',{max:40})};
}
function normalizeParser(value){
  ...
  return{
    version:text(parser.version,'parser.version',{max:100}),
    modelProfile:text(parser.modelProfile,'parser.modelProfile',{max:200}),
    parsedAt:text(parser.parsedAt,'parser.parsedAt',{max:100})
  };
}
function normalizeEvidence(items,noteId){
  ...
  for(const [index,raw] of array(items,'evidence',{max:200}).entries()){
    ...
    ids.add(id);
    result.push({id,excerpt,speaker});
  }
  return result;
}
...
function normalizeSummary(value,evidenceSet){
  ...
  return{
    text:text(summary.text,'summary.text',{max:5000}),
    confidence:confidence(summary.confidence,'summary.confidence'),
    evidenceIds:evidenceRefs(summary.evidenceIds,'summary.evidenceIds',evidenceSet)
  };
}
function normalizeTopics(items){
  return array(items,'topics',{max:50}).map((raw,index)=>{
    const item=plainObject(raw,`topics[${index}]`);
    knownKeys(item,new Set(['text','confidence']),`topics[${index}]`);
    return{text:text(item.text,`topics[${index}].text`,{max:300}),confidence:confidence(item.confidence,`topics[${index}].confidence`)};
  });
}
function stableClaimId(noteId,kind,evidenceIds,textValue){
  return `${kind}_${digest(`${noteId}\0${canonicalEvidenceIds(evidenceIds).join('\0')}\0${normalizeWhitespace(textValue)}`).slice(0,24)}`;
}
function normalizeDecisions(items,noteId,evidenceSet){
  return array(items,'decisions',{max:100}).map((raw,index)=>{
    const item=plainObject(raw,`decisions[${index}]`);
    knownKeys(item,new Set(['text','status','confidence','evidenceIds']),`decisions[${index}]`);
    const claimText=text(item.text,`decisions[${index}].text`,{max:1000});
    ...
    const evidenceIds=evidenceRefs(item.evidenceIds,`decisions[${index}].evidenceIds`,evidenceSet);
    return{id:stableClaimId(noteId,'decision',evidenceIds,claimText),text:claimText,status,confidence:confidence(item.confidence,`decisions[${index}].confidence`),evidenceIds};
  });
}
function normalizeActionCandidates(items,noteId,evidenceSet){
  const seen=new Set();
...
function normalizeRisks(items,noteId,evidenceSet){
  return array(items,'risks',{max:100}).map((raw,index)=>{
    const item=plainObject(raw,`risks[${index}]`);
    knownKeys(item,new Set(['text','severity','confidence','evidenceIds']),`risks[${index}]`);
    const claimText=text(item.text,`risks[${index}].text`,{max:1000});
    ...
    const evidenceIds=evidenceRefs(item.evidenceIds,`risks[${index}].evidenceIds`,evidenceSet);
    return{id:stableClaimId(noteId,'risk',evidenceIds,claimText),text:claimText,severity,confidence:confidence(item.confidence,`risks[${index}].confidence`),evidenceIds};
  });
}
function normalizeQuestions(items,noteId,evidenceSet){
  return array(items,'openQuestions',{max:100}).map((raw,index)=>{
    const item=plainObject(raw,`openQuestions[${index}]`);
    knownKeys(item,new Set(['text','confidence','evidenceIds']),`openQuestions[${index}]`);
    const claimText=text(item.text,`openQuestions[${index}].text`,{max:1000});
    const evidenceIds=evidenceRefs(item.evidenceIds,`openQuestions[${index}].evidenceIds`,evidenceSet);
    return{id:stableClaimId(noteId,'question',evidenceIds,claimText),text:claimText,confidence:confidence(item.confidence,`openQuestions[${index}].confidence`),evidenceIds};
  });
}
function normalizeProjects(items,noteId,evidenceSet){
  return array(items,'projectCandidates',{max:50}).map((raw,index)=>{
    const item=plainObject(raw,`projectCandidates[${index}]`);
    knownKeys(item,new Set(['name','confidence','evidenceIds']),`projectCandidates[${index}]`);
    const name=text(item.name,`projectCandidates[${index}].name`,{max:300});
    const evidenceIds=evidenceRefs(item.evidenceIds,`projectCandidates[${index}].evidenceIds`,evidenceSet);
    return{id:stableClaimId(noteId,'project',evidenceIds,name),name,confidence:confidence(item.confidence,`projectCandidates[${index}].confidence`),evidenceIds};
  });
}
function normalizeQuality(value={}){
  const quality=plainObject(value,'quality');
  knownKeys(quality,new Set(['overallConfidence','warnings']),'quality');
  return{
    overallConfidence:confidence(quality.overallConfidence,'quality.overallConfidence'),
    warnings:array(quality.warnings,'quality.warnings',{max:50}).map((item,index)=>text(item,`quality.warnings[${index}]`,{max:500}))
  };
}
...
export function normalizeGetnoteInsight(value){
  ...
  return{
    ...
    decisions:normalizeDecisions(input.decisions,note.noteId,evidenceSet),
    actionCandidates:normalizeActionCandidates(input.actionCandidates,note.noteId,evidenceSet),
    risks:normalizeRisks(input.risks,note.noteId,evidenceSet),
    openQuestions:normalizeQuestions(input.openQuestions,note.noteId,evidenceSet),
    projectCandidates:normalizeProjects(input.projectCandidates,note.noteId,evidenceSet),
    evidence,
...
export class GetnoteInsightStore{
  constructor(dataDir){
    ...
    this.root=path.join(this.dataDir,'getnote');
    this.insightDir=path.join(this.root,'insights');
    this.indexFile=path.join(this.root,'index.json');
    this.candidateFile=path.join(this.root,'candidates.json');
  ...
  async _ensureDirectory(target,label){
    const existing=await this._stat(target,label,'directory');
    if(!existing)await fsp.mkdir(target,{recursive:true,mode:DIRECTORY_MODE});
    await this._stat(target,label,'directory');
  ...
  async _ensureJsonFile(target,label,initial){
    ...
    if(!existing){
      try{await fsp.writeFile(target,JSON.stringify(initial,null,2),{encoding:'utf8',flag:'wx',mode:FILE_MODE});}
      catch(error){if(error.code!=='EEXIST')throw error;}
    }
    await this._stat(target,label,'file');
    await fsp.chmod(target,FILE_MODE);
  }
  async ensure(){
    await this._ensureDirectory(this.dataDir,'DATA_DIR');
    await this._ensureDirectory(this.root,'getnote');
    await this._ensureDirectory(this.insightDir,'getnote/insights');
    await this._ensureJsonFile(this.indexFile,'getnote/index.json',emptyIndex());
    await this._ensureJsonFile(this.candidateFile,'getnote/candidates.json',emptyCandidates());
  }
  async _readJson(target,label){
    await this.ensure();
    await this._stat(target,label,'file');
    try{return JSON.parse(await fsp.readFile(target,'utf8'));}
    catch(error){throw new GetnoteInsightError(`${label} 无法读取为 JSON。`,{code:'GETNOTE_INSIGHT_STORE_CORRUPT',statusCode:500,cause:error});}
  }
  async _atomicWrite(target,label,value){
    ...
    let created=false;
    try{
      await fsp.writeFile(tmp,JSON.stringify(value,null,2),{encoding:'utf8',flag:'wx',mode:FILE_MODE});
      created=true;
  ...
  async _writeImmutable(target,label,value){
    ...
    try{
      await fsp.writeFile(target,JSON.stringify(value,null,2),{encoding:'utf8',flag:'wx',mode:FILE_MODE});
      await fsp.chmod(target,FILE_MODE);
      return true;
  ...
  _validateIndex(index){
    if(!index||index.schemaVersion!==GETNOTE_INSIGHT_INDEX_VERSION||!index.entries||!index.latestByNote)throw new GetnoteInsightError('GetNote insight index 格式无效。',{code:'GETNOTE_INSIGHT_STORE_CORRUPT',statusCode:500});
    return index;
  }
  ...
  _insightFilename(cacheKey){return `${cacheKey}.json`;}
  _safeInsightFile(filename){
    const raw=String(filename||'');
    if(path.basename(raw)!==raw||!/^gni_[a-f0-9]{40}\.json$/.test(raw))throw this._unsafe('insight file','文件名无效');
    return path.join(this.insightDir,raw);
  }

  async findCachedInsight({noteId,contentHash:hash,parserVersion,modelProfile}={}){
    const cacheKey=insightCacheKeyFor({noteId,contentHash:hash,parserVersion,modelProfile});
    ...
    const target=this._safeInsightFile(entry.file);
    const stat=await this._stat(target,entry.file,'file');
    if(!stat)throw new GetnoteInsightError('GetNote insight index 指向的缓存文件不存在。',{code:'GETNOTE_INSIGHT_STORE_CORRUPT',statusCode:500});
  ...
  async putInsight(value){
    return this._enqueue(async()=>{
      ...
      const filename=this._insightFilename(cacheKey);
      const target=this._safeInsightFile(filename);
      await this.ensure();
      const created=await this._writeImmutable(target,filename,insight);
      let replayed=!created;
      if(!created){
        const existing=normalizeGetnoteInsight(JSON.parse(await fsp.readFile(target,'utf8')));
        if(!sameJson(existing,insight))throw new GetnoteInsightError('相同 GetNote cache tuple 已存在不同解析结果，拒绝静默覆盖。',{code:'GETNOTE_INSIGHT_CACHE_CONFLICT',statusCode:409});
      }
      const index=this._validateIndex(await this._readJson(this.indexFile,'getnote/index.json'));
      index.entries[cacheKey]={
        cacheKey,file:filename,noteId:insight.note.noteId,title:insight.note.title,contentHash:insight.source.contentHash,
        parserVersion:insight.parser.version,modelProfile:insight.parser.modelProfile,parsedAt:insight.parser.parsedAt
      };
      index.latestByNote[insight.note.noteId]=cacheKey;
      await this._atomicWrite(this.indexFile,'getnote/index.json',index);
      const candidates=await this._reconcileCandidates(insight);
      return{cacheKey,insight,replayed,candidates};
    });
  }
  ...
  _candidateSnapshot(insight,candidate){
    return{
      ...
      noteId:insight.note.noteId,
      noteTitle:insight.note.title,
      noteUrl:insight.note.noteUrl,
      sourceContentHash:insight.source.contentHash,
      parserVersion:insight.parser.version,
      modelProfile:insight.parser.modelProfile,
      text:candidate.text,
      ownerHint:candidate.ownerHint,
      dueHint:candidate.dueHint,
      explicitDueDate:candidate.explicitDueDate,
      confidence:candidate.confidence,
      evidenceIds:[...candidate.evidenceIds]
    };
  }
  async _reconcileCandidates(insight){
    const store=this._validateCandidates(await this._readJson(this.candidateFile,'getnote/candidates.json'));
    const timestamp=nowIso();
    const current=new Set();
    for(const candidate of insight.actionCandidates){
      const snapshot=this._candidateSnapshot(insight,candidate);
      current.add(candidate.candidateKey);
      const existing=store.items[candidate.candidateKey];
      if(existing){
        const restored=existing.state==='stale';
        Object.assign(existing,snapshot,{sourcePresent:true,lastSeenAt:timestamp});
        if(restored){existing.state='pending';existing.staleAt=null;}
      }else{
        store.items[candidate.candidateKey]={...snapshot,state:'pending',sourcePresent:true,firstSeenAt:timestamp,lastSeenAt:timestamp,reviewedAt:null,staleAt:null,sourceChangedAt:null,resolution:null};
      }
    }
    for(const item of Object.values(store.items)){
      ...
      if(item.state==='pending'){
        ...
        item.sourceChangedAt=timestamp;
      }
    }
  ...
  async listCandidates({states=null,noteId=null}={}){
    const store=this._validateCandidates(await this._readJson(this.candidateFile,'getnote/candidates.json'));
    const allowed=states===null?null:new Set(Array.isArray(states)?states:[states]);
    return Object.values(store.items)
      .filter(item=>(!allowed||allowed.has(item.state))&&(!noteId||item.noteId===noteId))
  ...
  _normalizeResolution(value,state){
    ...
    const resolution=plainObject(value,'candidate resolution');
    knownKeys(resolution,new Set(['type','id']),'candidate resolution');
    const type=text(resolution.type,'candidate resolution.type',{max:40});
  ...
  async setCandidateReviewState(candidateKey,{expectedState,state,resolution=null}={}){
    return this._enqueue(async()=>{
      const key=text(candidateKey,'candidateKey',{max:80});
      const nextState=text(state,'candidate state',{max:20});
      const expected=text(expectedState,'candidate expectedState',{max:20});
      if(!HUMAN_STATES.has(nextState)||nextState==='stale')fail('人工审核状态无效。','INVALID_GETNOTE_CANDIDATE_REVIEW');
      const store=this._validateCandidates(await this._readJson(this.candidateFile,'getnote/candidates.json'));
      ...
      item.resolution=this._normalizeResolution(resolution,nextState);
      if(nextState==='pending')item.reviewedAt=null;
      await this._atomicWrite(this.candidateFile,'getnote/candidates.json',store);
